...
int main(int argc, char ** argv)
{
    ...
    auto a_k = M->variable( new_array_ptr<int,1>({A,A})) ;
    auto C_param = M->parameter(new_array_ptr<int,1>({(A+I)*(A+I)})) ;
    auto weights_param = M->parameter(new_array_ptr<int,1>({A*A})) ;
    ...
    TA_resulted_rates = Expr::sub(TA_resulted_rates, a_k);
    auto c_task_agents = M->constraint(TA_resulted_rates, Domain::greaterThan(0.));
   
    
    // Define Objective
    auto a_k_resh = Expr::reshape(a_k,A*A);
    M->objective(ObjectiveSense::Maximize, Expr::dot(a_k_resh,weights_param));
