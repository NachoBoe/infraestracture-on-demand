...
void update_positions(std::vector<std::vector<double>> agents, std::vector<std::vector<double>> NA, int nmb_agents, int nmb_NA, int Px,auto dual_vars,double lr, std::vector<std::vector<double>>(*NA_ptr));
...
void get_directions(std::vector<std::vector<double>> agents, std::vector<std::vector<double>> NA, int nmb_agents, int nmb_NA, int Px,auto dual_vars,double max_delta, std::vector<std::vector<double>>(*NA_directions_ptr));
    ...
    for (i=0;i<nmb_agents;i++){
        for (j=0;j<nmb_agents;j++){
            if (j!=i){
                ...
                max_rates[nmb_agents*i + j] = exp(-1*Px*pow(d,2));
...
{   int i,j;
    ...
    for (i=nmb_agents-nmb_NA;i<nmb_agents;i++){
        for (j=0;j<nmb_agents;j++){
            ...
            if (1. >=  0.8 * 2.*Px*d*d){
                exponente = -1*(Px*2* exp(-1*pow(Px * d,2)));
            }
...
void simulate(std::vector<std::vector<double>> (*TA_ptr), std::vector<std::vector<double>> (*NA_ptr),int nmbr_TA, int nmbr_NA, std::vector<std::vector<double>> (*NA_directions_ptr),double sim_time,double deltaT, bool *finished_ptr, std::string path,std::vector<double> *W_ptr )
    /* Input:
...
{    
    ...
    double side = sqrt(nmbr_TA+nmbr_NA);
    ...
    for (int iter_t=0; iter_t < sim_time/deltaT;iter_t++){
        ...
        *NA_ptr = local_NA;
        ...
        if (iter_t % int(sim_time/deltaT/5) == 0){
            for (i=0; i < A; i++){
                W_ptr*[i] = 0;
            }
            for (i=0; i < A/5; i++){
                int ran_idx = std::rand() % 25;
                W_ptr*[ran_idx] = 1;
                sinks_file_ptr* << ran_idx ;
                if (i!= A/5-1){
                    sinks_file << ','
                }
            }
...
int main(int argc, char ** argv)
{
    ...
    for(int experiment=0;experiment<nmbr_experiments;experiment++){
        ...
        std::ofstream sinks_file;
        sinks_file.open (path + "/sinks_positions.txt",std::ios::out);
        ...
        double deltaT = 0.2;
        double max_vel = 30;
        std::thread th1(simulate, &TA,&NA,A,I,&NA_directions,sim_duration, deltaT,&finished,path,&W,&sinks_file);
        W_monty = monty::new_array_ptr<double>(W);
        weights_param->setValue(W_monty);
        ...
        while(finished==false){
            ...
            // Update NA positons
            get_directions(agents, NA, nmb_agents, I, Px, duals,max_vel*deltaT,&NA_directions);
            update_agents(TA,NA,A,I,&agents);
        }
